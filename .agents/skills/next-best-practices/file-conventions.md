...
Replaces `middleware.ts`. **Do not create both** — having both causes a build error.
...
|---------|------|--------|--------|
| v14-15 | `middleware.ts` | `export function middleware` | `config` |
| v16+ | `proxy.ts` | `export default function proxy` | `config` |
